    ...
    init: function () {
        ...
        const AlchTable = {
            ...
            cookieParts: {
                ...
                darkChocolateChips: {
                    ...
                    prefix: {
                        ...
                        flat: true,
                        qualityEffect(cookiePart, meQuality) {
                            cookiePart[2].forEach((v, i, a) => { if (v[0] === 'chocolate')
                                a[i][2] += meQuality * this.max; });
                            return cookiePart;
                        },
                        max: 0.25
                    }
            ...
            getSortedIngredients: function () {
                const ingredients = Object.keys(AlchTable.ingredients);
                return ingredients.sort((a, b) => AlchTable.ingredients[a].min - AlchTable.ingredients[b].min);
            },
            getSortedEffects: function () {
                const effects = Object.keys(AlchTable.effects).sort();
                return effects;
            },
            getSortedCookiePartTypes: function () {
                const cookiePartTypes = Object.keys(AlchTable.cookiePartTypes);
            ...
            getIcon: function (selected) {
                ...
                if (selected && selected[0]) {
                    ...
                    if (partId)
                        icon = AlchTable.cookieParts[partId].icon;
                }
            ...
            formatIngredient: function (effectTypes, text) {
                ...
                const color0 = effectTypes[0] === 'aura' ? "#9e7" : (effectTypes[0] ? AlchTable.ingredients[effectTypes[0]].color : undefined);
                const color1 = effectTypes[1] === 'aura' ? "#9e7" : (effectTypes[1] ? AlchTable.ingredients[effectTypes[1]].color : undefined);
                let style = "";
                if (color0 && color1)
                    style = `background-image: linear-gradient(90deg, ${color0} 40%, ${color1} 60%);-webkit-background-clip: text;-webkit-text-fill-color: transparent;text-shadow:none;`;
                else if (color0)
                    style = `color: ${color0};`;
            ...
            comboLookup: function (ingredient0, ingredient1) {
                ...
                for (const part of Object.keys(AlchTable.cookieParts)) {
            ...
            nextCookiePart: function (partId) {
                ...
                if (partInfo.combo[1] !== partInfo.combo[0])
                    for (let i = 0; i < Object.keys(AlchTable.cookieSuffixes[partInfo.combo[1]]).length; i++) {
                        suffixPool.push([
                            partInfo.combo[1],
                            i,
                ...
                if (Game.dragonLevel >= 5)
                    suffixPool.push(['aura', Game.dragonAura, AlchTable.cookieSuffixes.aura[Game.dragonAura].weight]);
                if (Game.dragonLevel >= 27)
                    suffixPool.push(['aura', Game.dragonAura2, AlchTable.cookieSuffixes.aura[Game.dragonAura2].weight]);
                ...
                for (let i = 0; i < numberOfSuffixes; i++) {
                    if (!suffixPool.length)
                        break;
                    let suffix = ['flour', 0, 0];
                ...
                const effectTypes = AlchTable.getSortedIngredients();
                effectTypes.push('aura');
                // TODO!!!
                suffixes.sort((a, b) => a[1] - b[1]).sort((a, b) => effectTypes.indexOf(a[0]) - effectTypes.indexOf(b[0]));
            ...
            chain: function (effs, affix, quality) {
                ...
                if ('flat' in effect)
                    effs[affix.effect] = (effs[affix.effect] ?? 0) + (affix.max * quality);
                else if ('inverse' in effect)
                    effs[affix.effect] = (effs[affix.effect] ?? 1) * (1 - affix.max * quality);
                else
                    effs[affix.effect] = (effs[affix.effect] ?? 1) * (1 + affix.max * quality);
            },
            calculateEffs: function () {
                ...
                if (parts.length) {
                    const newParts = parts.map(a => [a[0], a[1], a[2].map(b => [b[0], b[1], b[2]])]);
                    parts.forEach((part) => {
                    ...
                    newParts.forEach((part) => {
                        const prefix = AlchTable.cookieParts[part[0]].prefix;
                        const quality = part[1];
                        if (typeof prefix.effect === 'function')
                            prefix.effect(effs, quality);
                        else if (typeof prefix.effect === 'string')
                            AlchTable.chain(effs, prefix, quality);
                        part[2].forEach((suffixData) => {
                            const suffix = AlchTable.cookieSuffixes[suffixData[0]][suffixData[1]];
                            const suffixQuality = suffixData[2];
                            if (suffix && typeof suffix.effect === 'function')
                                suffix.effect(effs, suffixQuality);
                            else if (suffix && typeof suffix.effect === 'string')
                                AlchTable.chain(effs, suffix, suffixQuality);
                        });
                    });
                }
			...
			`;
                ...
                forge: function () {
                    ...
                    for (let i = 0; i < 2; i++) {
                        ...
                        if (selected && !selected[0])
                            me.style.backgroundPosition = AlchTable.ingredients[selected[1]].icon;
                        //TODO!!!
                        else if (selected && selected[0])
                            me.style.backgroundPosition = AlchTable.cookieParts[AlchTable.saveData.cookieParts[selected[0]]?.[selected[1]]?.[0]]?.icon || '';
                    }
                },
                cookie: function () {
                    ...
                    const me = l(`alchtableCookieInfo`);
                    if (!me)
                        return;
                ...
                parts: function () {
                    ...
                    for (let i of cookiePartTypeIds) {
                        for (let j = 0; j < 12; j++) {
                            const part = l(`alchtablePart-${i}-${j}`);
                            if (!part)
                                continue;
                            if (AlchTable.selected && AlchTable.selected[0] === i && AlchTable.selected[1] === j)
                                part.classList.add('selected');
                            else
                                part.classList.remove('selected');
                            if (AlchTable.saveData.enabledParts[i] === j)
                                part.classList.add('enabled');
                            else
                                part.classList.remove('enabled');
                            if (j >= (AlchTable.saveData.cookieParts[i]?.length ?? 0))
            ...
            callback: {
                ...
                whiteButton: function () {
                    ...
                    for (let i = ingredients.length - 1; i >= 0; i--) {
                        const ingredient = AlchTable.ingredients[ingredients[i]];
                        if (effectiveCrumbs < ingredient.min)
                            continue;
                        AlchTable.saveData.ingredients[ingredients[i]] = (AlchTable.saveData.ingredients[ingredients[i]] ?? 0) + 1;
                        break;
                    }
                    AlchTable.updateAll();
                },
                forgeButton: function () {
                    ...
                    if (!leftSlot || !rightSlot || !!leftSlot[0] !== !!rightSlot[0])
                        ...
                        if (!AlchTable.saveData.ingredients[ingredient0])
                            AlchTable.forgeSlots[0] = false;
                        if (!AlchTable.saveData.ingredients[ingredient1])
                            AlchTable.forgeSlots[1] = false;
                        else if (ingredient0 === ingredient1 && AlchTable.saveData.ingredients[ingredient0] < 2)
                            AlchTable.forgeSlots[1] = false;
                ...
                trashButton: function () {
                    if (!AlchTable.selected || !AlchTable.selected[0])
                        return;
                    ...
                    function func() {
                        ...
                        if (enabledPart === AlchTable.selected[1])
                            AlchTable.saveData.enabledParts[AlchTable.selected[0]] = -1;
                        else if (enabledPart > AlchTable.selected[1])
                            AlchTable.saveData.enabledParts[AlchTable.selected[0]] = enabledPart - 1;
            ...
            rebuild: function () {
                ...
                if (list) {
                    ...
                    ingredientIds.forEach(ingredientId => {
                        const me = { l: l(`alchtableIngredient-${ingredientId}`), events: {} };
                        if (!me.l)
                            return;
                        AlchTable.HTML.ingredients[ingredientId] = me;
                        [, , me.events['click']] = AddEvent(me.l, 'click', function (ingredientId) {
                            return function () {
                                if (!AlchTable.saveData.ingredients[ingredientId])
                                    return;
                                if (AlchTable.selected && !AlchTable.selected[0] && AlchTable.selected[1] === ingredientId)
                                    AlchTable.selected = false;
                                else
                                    AlchTable.selected = [false, ingredientId];
                        ...
                        return function () {
                            ...
                            if (selected) {
                                if ((thisSlot && selected[0] === thisSlot[0] && selected[1] === thisSlot[1])
                                    || (!selected[0] && !AlchTable.saveData.ingredients[selected[1]]))
                                    return;
                                if (otherSlot
                ...
                if (cookieBox) {
                    ...
                    cookiePartTypeIds.forEach(v => {
                        ...
                        AddEvent(slotL, 'click', function (v) {
                            return function () {
                                if (!AlchTable.selected)
                                    AlchTable.saveData.enabledParts[v] = -1;
                                else if (!AlchTable.selected[0])
                                    return;
                                else {
                ...
                if (box) {
                    ...
                    for (let partType of cookiePartTypeIds) {
                        ...
                        const cookiePartHTMLs = [];
                        for (let j = 0; j < 12; j++) {
                            const me = { l: l(`alchtablePart-${partType}-${j}`), events: {} };
                            if (!me.l)
                                return;
                            cookiePartHTMLs[j] = me;
                            [, , me.events['click']] = AddEvent(me.l, 'click', function (i, j) {
                                return function () {
                                    if (!AlchTable.saveData.cookieParts[i]?.[j])
                                        return;
                                    if (AlchTable.selected && AlchTable.selected[0] === i && AlchTable.selected[1] === j)
                                        AlchTable.selected = false;
                                    else
                                        AlchTable.selected = [i, j];
                                    PlaySound('snd/toneTick.mp3');
                                    AlchTable.updateAll();
                                };
                            }(partType, j));
                ...
                AlchTable.HTML.cursor.l = l('alchtableCursor');
            },
            tooltip: {
                ingredient: function (id) {
					...
					`;
                        function rarityHint(weight) {
                            if (weight <= 5)
                                return `<span style="font-size:8pt;color:#9e7">&lt;R&gt; </span>`;
                            if (weight < 25)
                                return `<span style="font-size:8pt;color:#97f">&lt;U&gt; </span>`;
                ...
                forge: function () {
                    return function () {
                        ...
                        if (!AlchTable.forgeSlots[0] || !AlchTable.forgeSlots[1] || !!AlchTable.forgeSlots[0][0] !== !!AlchTable.forgeSlots[1][0]) {
                            ...
                            let partIds = {};
                            combos.forEach((v) => { if (AlchTable.comboLookup(...v))
                                partIds[AlchTable.comboLookup(...v)] = 1 + (partIds[AlchTable.comboLookup(...v)] ?? 0); });
                            Object.keys(partIds).sort((a, b) => {
                                const ings = AlchTable.getSortedIngredients();
                                const comboA = AlchTable.cookieParts[a].combo;
                                const comboB = AlchTable.cookieParts[b].combo;
                                if (comboA[0] === comboB[0])
                                    return ings.indexOf(comboA[1]) - ings.indexOf(comboB[1]);
                                else
                                    return ings.indexOf(comboA[0]) - ings.indexOf(comboB[0]);
                            }).forEach(partId => {
                                if (!partId)
                                    return;
                                const part = AlchTable.cookieParts[partId];
                                ...
                                str += /*html*/ `
									<div style="white-space: nowrap;">
										<div style="font-size:8pt;color:#aaa;display:inline-block;">&lt;${partIds[partId]}/6&gt;</div>
										<div class="alchtableThingIcon" style="display:inline-block;background-position:${part.icon};margin:-6px -14px -16px;transform:scale(0.5);position:relative;"></div>
										<div class="name" style="display:inline-block;">${part.name}: </div>
										<div style="display:inline-block;">${affixString}</div>
									</div>
                            ...
                            const ingredient0 = AlchTable.forgeSlots[0][1];
                            const ingredient1 = AlchTable.forgeSlots[1][1];
                            const partId = AlchTable.comboLookup(ingredient0, ingredient1);
                            if (!partId)
                                return str + '</div>';
                            const part = AlchTable.cookieParts[partId];
                ...
                part: function (type, number) {
                    return function () {
						...
						<div style="padding:8px 4px;min-width: 350px;">
                            ...
                            + (combo[1] !== combo[0] ? (" & " + AlchTable.formatIngredient(combo[1])) : '')}</div>
							<div class="line"></div>
            ...
            draw: function () {
                ...
                if (AlchTable.HTML.cursor.l) {
                    if (!AlchTable.HTML.cursor.show || !AlchTable.selected || (AlchTable.selected[0] && !AlchTable.saveData.cookieParts[AlchTable.selected[0]]?.[AlchTable.selected[1]]))
                        AlchTable.HTML.cursor.l.style.display = 'none';
                    else {
        ...
        Game.eff = function (name, def) {
            def = def ?? (Game.effects[name]?.flat ? 0 : 1);
            ...
            for (let name in Game.Objects) {
                if (Game.Objects[name].minigameLoaded && Game.Objects[name].minigame.effs) {
                    for (let eff in Game.Objects[name].minigame.effs) {
                        if (Game.effects[eff]?.flat)
                            effs[eff] = (effs[eff] ?? 0) + (Game.Objects[name].minigame.effs[eff] ?? 0);
                        else
                            effs[eff] = (effs[eff] ?? 1) * (Game.Objects[name].minigame.effs[eff] ?? 1);
                    }
                }
            }
            Game.effs = effs;
