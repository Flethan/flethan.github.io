...
type EffectType = IngredientID | 'aura';
type Unit = 0 | 1 | 2 | 3 | 4 | 5;
type CookieType = 0 | 1 | 2;

type StaticEffect  = 'cps' | 'click' | 'milk' | 'goldenCookieGain' | 'goldenCookieFreq' | 'goldenCookieEffDur' | 'seasonPrice' | 'seasonUpgradePrice' | 'seasonSwitchGoldenCookie' | 'seedCost' | 'sugarFrenzyRefund' | 'lumpIceCreamBuff' | 'lumpsRipenTime'
type MetaEffect    = 'qualityMult' | 'pbQualityMult' | 'chocQualityMult' | 'deconstructAmount' | 'ingredientDivisor' | 'ccpsm' | 'ccpsa'
type SpecialEffect = 'iceCream';
type FlatEffect    = 'suckRate';
type Effect = StaticEffect | MetaEffect;
type Effects = Record<Effect, number>;
type FlatEffects = Record<FlatEffect, number>;
...
interface CookieEffect {
	effsStr: string,
	effect?: StaticEffect | SpecialEffect | ((effs: Effects, quality: number) => void),
	qualityEffect?: 'all' | 'chocolate',
	max?: number,
	unit?: Unit,
	weight?: number,
	aura?: string,
}
type CookieEffectData = [
	EffectType, //id
	number, //index
	number, //quality
]
...
type CookiePartData = [
	string, //id
	number, //quality
	CookieEffectData[] //affixes
]

interface SaveData {
	cookieCrumbs: number,
	ingredients: {
		[K in IngredientID]?: number
	},
	cookieParts: [CookiePartData[],CookiePartData[],CookiePartData[]],

	deconstructing: boolean,
	inputValue: number,
	selectedIngredients: [number, number],
	selectedParts: [number, number, number],
	effectsOn: boolean,
...
interface AlchTable {
	...
	saveData: SaveData,
	effs: Effects,
	flatEffs: FlatEffects,
	ingredients: Record<IngredientID, Ingredient>,
	cookieParts: Record<string, CookiePart>,
	cookieEffects: Record<EffectType, CookieEffect[]>,

	ccps: number,
	suckedPs: number,
	suckRate: number,
	ingredientSelected: IngredientID | null,
	forgeSlot0: IngredientID | null,
	forgeSlot1: IngredientID | null,
	cursor: boolean,
	cursorL: HTMLElement,

	getIngredients: () => [IngredientID, number][],
	formatEffect: (effect: CookieEffect, q: number) => [string, string],
	formatIngredient: (ingredientNames: EffectType | EffectType[] | null, text?: string) => string,
	cookiePartLookup: (ingredient0: IngredientID, ingredient1?: IngredientID) => CookiePart,
	nextCookiePart: (partInfo: CookiePart) => CookiePartData,
	calculateEffs: () => void,

	tooltip: {
		ingredient: (id: IngredientID) => () => string,
		part: (type: CookieType, number: number) => () => string,
	},
...
Game.registerMod("Alchemists Table Minigame", {
	init: function(){
		...
		AlchTable.getIngredients = function () {
			const ingredients: [IngredientID, number][] = [];
			for (const ingredient in AlchTable.ingredients) {
				ingredients.push([ingredient as IngredientID, AlchTable.ingredients[ingredient].min]);
			}
		...
		AlchTable.formatEffect = function (effect, q) {
			...
			if (effect.unit === 0) withUnits = `${Beautify(number, 1)}`;
			if (effect.unit === 1) withUnits = `${Beautify(number * 100, 1)}%`;
			if (effect.unit === 2) withUnits = `${Beautify(Math.floor(number), 1)} minutes`;
			if (effect.unit === 3) withUnits = `${Beautify(number, 1)} times`;
			if (effect.unit === 4) withUnits = `${Beautify(Math.ceil(number), 1)}`;
			if (effect.unit === 5) withUnits = `${Beautify(number * 10000, 1)}‱`;
		...
		AlchTable.formatIngredient = function (effectTypes, text) {
			...
			const color0: HexColor = effectTypes[0] === 'aura' ? "#9e7" : AlchTable.ingredients[effectTypes[0]]?.color;
			const color1: HexColor = effectTypes[1] === 'aura' ? "#9e7" : AlchTable.ingredients[effectTypes[1]]?.color;
			let style = "";
			if (color0 && color1) style = `background-image: linear-gradient(90deg, ${color0}, ${color1});-webkit-background-clip: text;-webkit-text-fill-color: transparent;text-shadow:none;`;
		...
		AlchTable.nextCookiePart = function (partInfo: CookiePart) {
			Math.seedrandom(Game.seed+'/'+AlchTable.saveData.totalCookieParts);

			const baseId = partInfo.id;
			const baseQuality = Math.random();
			const affixes: CookieEffectData[] = [];

			const affixPool: [EffectType, number, number][] = []
			...
			for (let i = 0; i < 3; i++) {
				...
				let affix: CookieEffectData = ['flour',0,0];
			...
			const effectTypes: EffectType[] = AlchTable.getIngredients().map((v) => v[0] as EffectType);
		...
		AlchTable.update = {
			...
			ingredients: function () {
				if (l('alchtableIngredientsList')) {
					const ingredientIds: IngredientID[] = Object.keys(AlchTable.ingredients) as IngredientID[];
					ingredientIds.sort((a, b) => AlchTable.ingredients[a].min - AlchTable.ingredients[b].min);
					for (let i = 0; i < ingredientIds.length; i++) {
						const ingredientId = ingredientIds[i];
						if (!AlchTable.ingredients[ingredientId].l) continue;
			...
			forge: function () {
				for (let i = 0; i < 2; i++) {
					...
					if (!me) continue;
					const ingredient = AlchTable[`forgeSlot${i}`] as IngredientID | null;
					ingredient ? me.classList.add('on') : me.classList.remove('on');
					if (ingredient)	me.style = `background-position: ${AlchTable.ingredients[ingredient].icon}`;
				}
			},
		...
		AlchTable.callback = {
			...
			whiteButton: function () {
				...
				for (let i = ingredients.length - 1; i >= 0; i--) {
					if (effectiveCrumbs < ingredients[i][1]) continue;
			...
			forgeButton: function () {
				...
				if (!AlchTable.saveData.ingredients[AlchTable.forgeSlot0]) AlchTable.forgeSlot0 = null;
				if (!AlchTable.saveData.ingredients[AlchTable.forgeSlot1]) AlchTable.forgeSlot1 = null;
				if ((AlchTable.forgeSlot0 === AlchTable.forgeSlot1 && AlchTable.saveData.ingredients[AlchTable.forgeSlot0] < 2)) AlchTable.forgeSlot1 = null;
				Math.seedrandom(Game.seed + '/' + AlchTable.saveData.totalCookieParts);
		...
		AlchTable.buildIngredients = function () {
			...
			for (let i = 0; i < ingredientIds.length; i++) {
				...
				AddEvent(me.l, 'click', function (ingredientId) {
					return function () {
						if (!AlchTable.saveData.ingredients[ingredientId] && !Game.sesame) return false;
						if (AlchTable.ingredientSelected === ingredientId) AlchTable.ingredientSelected = null;
						else { 
				...
				AddEvent(me, 'click', function (i) {
					return function () {
						...
						if (AlchTable.saveData.ingredients[AlchTable.ingredientSelected] < 2 &&
							AlchTable[`forgeSlot${1-i}`] === AlchTable.ingredientSelected) AlchTable[`forgeSlot${1-i}`] = null;
						if (!Game.keys[16]) AlchTable.ingredientSelected = null;
						AlchTable.update.ingredients();
						AlchTable.update.forge();
				}}(i));
			}
		...
		AlchTable.draw = function () {
			...
			if (AlchTable.cursorL) {
				if (!AlchTable.cursor || !AlchTable.ingredientSelected) AlchTable.cursorL.style.display='none';
				else {
