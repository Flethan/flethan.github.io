...
Game.registerMod("Alchemists Table Minigame", {
	init: function(){
		...
		AlchTable.getIngredientMins = function () {
			const ingredientMins = [];
			for (const ingredient in AlchTable.ingredients) {
				ingredientMins.push(ingredient.min);
			}
			return ingredientMins.toSorted((a, b) => a - b).map(v => v / (AlchTable.effs.ingredientDivisor || 1));
		};
		...
		AlchTable.formatEffect = function (effect, q) {
			...
			return [effect.effsStr.replace('@', `<span style="color: #bbb">${number}</span>`), `(${Beautify(q * 100, 1)}%)`];
		};

		AlchTable.formatIngredient = function (ingredientNames, text) {
			if (!Array.isArray(ingredientNames)) ingredientNames = [ingredientNames];
			const color0 = ingredientNames[0] === 'aura' ? "#9e7" : AlchTable.ingredients[ingredientNames[0]]?.color;
			const color1 = ingredientNames[1] === 'aura' ? "#9e7" : AlchTable.ingredients[ingredientNames[1]]?.color;
			let style = "";
			if (color0 && color1) style = `background-image: linear-gradient(90deg, ${color0}, ${color1}); background-clip: text; color: transparent;`;
			else if (color0) style = `color: ${color0};`;
			let str = /*html*/`<span style="${style}">${text??ingredientNames[0]??""}</span>`;
				...
				return function() {
						...
						<div style="padding:8px 4px;min-width: 350px;" id="alchtableTooltipPart">
							<div style="display: flex; justify-content: center;">
								...
								<div style="display: flex; justify-content: center; align-content: center; flex-direction: column;">
									...
									<div>${AlchTable.formatIngredient(partInfo.combo[0])} + ${AlchTable.formatIngredient(partInfo.combo[1])}</div>
								</div>
							</div>
							<div class="line"></div>
							<div class="alchtableBoxTitle title">${AlchTable.formatIngredient(partInfo.combo,AlchTable.formatEffect(partInfo, partData[1])[0])}
					...
					if (partData[2]?.length) {
						...
						for (let i = 0; i < partData[2].length; i++) {
							...
							str += /*html*/`
							<div class="alchtableBoxTitle title">${AlchTable.formatIngredient(subPartData[0],AlchTable.formatEffect(subPartInfo, subPartData[2])[0])}
		...
		AlchTable.callback = {
			...
			forgeButton: function () {
				...
				if (!AlchTable.saveData.ingredients[AlchTable.forgeSlot0]) AlchTable.forgeSlot0 = -1;
				if (!AlchTable.saveData.ingredients[AlchTable.forgeSlot1]) AlchTable.forgeSlot1 = -1;
				if ((AlchTable.forgeSlot0 === AlchTable.forgeSlot1 && AlchTable.saveData.ingredients[AlchTable.forgeSlot0] < 2)) AlchTable.forgeSlot1 = -1;
				...
				AddEvent(me, 'click', function (i) {
					return function () {
						...
						if (AlchTable.saveData.ingredients[AlchTable.ingredientSelected] < 2 &&
							AlchTable[`forgeSlot${1-i}`] === AlchTable.ingredientSelected) AlchTable[`forgeSlot${1-i}`] = -1;
						if (!Game.keys[16]) AlchTable.ingredientSelected = -1;
						AlchTable.update.ingredients();
						AlchTable.update.forge();
				}}(i));
			}
